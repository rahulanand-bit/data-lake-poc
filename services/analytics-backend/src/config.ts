...
import type { ModelMode, ModelProvider } from "./types.js";

export const cfg = {
  backendPort: Number(process.env.BACKEND_PORT ?? "4000"),
  corsOrigin: process.env.CORS_ORIGIN ?? "http://localhost:3001",
  modelMode: (process.env.MODEL_MODE ?? "stub") as ModelMode,
  primaryProvider: (process.env.PRIMARY_MODEL_PROVIDER ?? "gemini") as ModelProvider,
