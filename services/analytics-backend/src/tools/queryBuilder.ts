...
import type { ToolInput, ToolName, UserContext } from "../types.js";
...
const metricSqlMap: Record<string, string> = {
...
function normalizeDimension(field: string): string | null {
  if (allowedDimensions.has(field)) return field;
  if (field === "date" || field === "updated_date") return "DATE(updated_at)";
  return null;
}

function applyWhereClauses(input: ToolInput, user: UserContext, values: Array<string | number>): string {
...
export function buildQuery(tool: ToolName, input: ToolInput, user: UserContext): { sql: string; values: Array<string | number> } {
  ...
  if (tool === "overview_tool") {
    const groupBy = (input.group_by ?? []).map(normalizeDimension).filter(Boolean) as string[];
